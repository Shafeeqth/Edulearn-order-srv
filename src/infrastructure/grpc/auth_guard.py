...
class AuthGuard(IAuthGuard):
    ...
    async def authorize(self, user_info: dict, required_role: str = "STUDENT") -> None:
        ...
        if role != required_role:
            self.logger.error(f"Authorization failed: user role {role} does not match required role {required_role}")
            raise ValueError(f"User role {role} not authorized for this action")