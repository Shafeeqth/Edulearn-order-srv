...
class OrderSuccessUseCase:
    ...
    async def execute(self, dto: OrderPaymentSuccessDto):
        self.logger.info(f"Processing successful payment for order {dto.order_id}")

        async with get_db() as session:
            order = await self.order_repository.find_by_id(dto.order_id, session)
            if not order:
                raise OrderNotFoundException(f"Order not found: {dto.order_id}")
            ...
            if not order.payment_details:
                ...
                order.payment_details.setStatus("success")
        ...
        self.logger.info(f"✅ Order {dto.order_id} marked as COMPLETED")
        return
